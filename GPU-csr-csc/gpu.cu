...
const char *spgemm_desc = "GPU CSR-CSC SpGEMM";
...
__global__ void spgemm_kernel(int *d_A_row_ptrs, int *d_A_col_indices, double *d_A_values,
                              int *d_B_col_ptrs, int *d_B_row_indices, double *d_B_values,
                              double *d_result, int rows_A, int cols_B)
{
    ...
    for (int k = d_A_row_ptrs[row]; k < d_A_row_ptrs[row + 1]; ++k)
    {
        ...
        for (int l = d_B_col_ptrs[col]; l < d_B_col_ptrs[col + 1]; ++l)
        {
            if (d_B_row_indices[l] == col_idx)
            {
...
void init_spgemm(const sparse_CSR_t &A, const sparse_CSC_t &B)
{
    ...
    cudaMalloc(&d_B_col_ptrs, sizeof(int) * (B.rows + 1));
    cudaMemcpy(d_B_col_ptrs, B.col_ptrs.data(), sizeof(int) * (B.cols + 1), cudaMemcpyHostToDevice);
    ...
    cudaMalloc(&d_B_row_indices, sizeof(int) * (B.values.size()));
    cudaMemcpy(d_B_row_indices, B.row_indices.data(), sizeof(int) * (B.values.size()), cudaMemcpyHostToDevice);
...
void cleanup_spgemm()
{
    ...
    cudaFree(d_B_col_ptrs);
    cudaFree(d_A_col_indices);
    cudaFree(d_B_row_indices);
    cudaFree(d_A_values);
    cudaFree(d_B_values);
}
...
sparse_CSR_t spgemm(const sparse_CSR_t &A, const sparse_CSC_t &B)
{
    ...
    spgemm_kernel<<<blks, NUM_THREADS>>>(d_A_row_ptrs, d_A_col_indices, d_A_values,
                                         d_B_col_ptrs, d_B_row_indices, d_B_values,
